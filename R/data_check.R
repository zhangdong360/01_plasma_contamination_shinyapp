data_check <- function(data, data_group, cutoff = 0.9) {
  ...
  list_erythrocyte <- data.frame(GN = c("HBA1","HBB","CA1","HBD","PRDX2","CA2",
  ...
  list_platelet <- data.frame(GN = c("FLNA","TLN1","MYH9","ACTB","VCL",
  ...
  list_coagulation <- data.frame(GN = c("FGB","FGG","FGA","F13A1",
                                        "SERPINC1","PPBP","F2",
                                        "GP1BA","ECM1","CLU",
                                        "DSP","WDR1","ATRN",
                                        "GP5","SERPINA5","C1RL",
  ...
  # 转换为长格式
  data_ggplot <- tidyr::gather(data_merge, key = "key", value = "value", -c("id", "group"))
  ...
  marker_list <- list(
    ...
    coagulation = filtered_keys_coagulation,
    platelet = filtered_keys_platelet
  )
